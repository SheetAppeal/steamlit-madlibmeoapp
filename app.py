...
def generate_draft_image(text, to_val, subj_val):
    ...
    # 1. Auto-download modern fonts if they aren't on the server yet
    ...
    # 2. Set up the canvas (Matching your app's background color)
    width, height = 850, 950
    paper = Image.new('RGB', (width, height), (248, 250, 252)) # #f8fafc
    ...
    try:
        ...
        font_watermark = ImageFont.truetype(bold_font_path, 18)
    except:
        font_main = font_header = font_title = font_watermark = ImageFont.load_default()

    # 3. Draw a white "Email Card" with a subtle border
    draw.rectangle([(40, 40), (width - 40, height - 70)], fill=(255, 255, 255), outline=(226, 232, 240), width=2)
    ...
    for line in clean_text.split('\n'):
        if line.strip() == "":
            y += 15 # Add a little space for paragraph breaks
            continue
        for w_line in textwrap.wrap(line, width=58):
            draw.text((margin, y), w_line, font=font_main, fill=(30, 41, 59))
            y += 32
        y += 15
        
    # 7. Add the Brand Watermark at the very bottom
    ...
    draw.text(((width - text_width) / 2, height - 45), watermark_text, font=font_watermark, fill=(14, 165, 233))
...
if st.session_state.email_data:
    # Your updated bolding rule (no blue color!)
    body_html = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', st.session_state.email_data)
    
    # NEW: Convert the AI's text line breaks into HTML line breaks
